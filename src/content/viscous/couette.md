...
**出発点は運動量保存（[ナビエ・ストークス方程式](#/navier-stokes)）＋構成則 $\tau=\mu\,du/dy$ のふたつだけ。** 新しい物理は持ち込まず、上の条件（圧力勾配0・壁駆動）を入れて項を消すのがこのページの仕事。

## ステップ1: NSを簡略化する

[ナビエ・ストークス方程式](#/navier-stokes) の $x$ 成分から、効かない項を順に落とす:
...
ニュートン流体の $\tau=\mu\,du/dy$（[せん断応力](#/shear-stress)）に入れると、勾配が一定なので応力も一定:
...
- **🧱 材力（せん断の弾性↔粘性）**: 一定せん断 $\tau=\mu U/h$ は、材力の弾性せん断 $\tau=G\gamma$（$\gamma$＝変位勾配）の**“ひずみ→ひずみ速度”版**。変位勾配が速度勾配 $du/dy$ に替わっただけで、一定せん断のもとでは弾性が線形変位、粘性が線形速度（直線プロファイル）になる、という対応がそのまま効く。
- **🔥⚡ 拡散方程式の同型**: $d^2u/dy^2=0$ は**ソース無しの定常拡散**＝1次元ラプラス方程式。両端の温度を固定した定常・無発熱の熱伝導 $d^2T/dy^2=0$（→直線温度分布）と同型で、クエットの直線速度は「両端を固定した定常拡散」そのもの。右辺（ソース）が付くと[ポアズイユ](#/poiseuille-plates)の放物線になる。
