...
**出発点は運動量保存（[ナビエ・ストークス方程式](#/navier-stokes)）＋構成則 $\tau=\mu\,du/dy$。** 新しい物理は足さず、駆動を「圧力勾配」から「重力 $\rho g\sin\theta$」に差し替え、自由表面の応力ゼロ条件を入れるだけ。

## ステップ1: NSを簡略化する（重力が駆動）

[ナビエ・ストークス方程式](#/navier-stokes) の $x$ 成分。重力の $x$ 成分は $g_x=g\sin\theta$。定常・完全発達で慣性項を落とし、$dp/dx=0$ も落とすと:
...
壁で0、表面で最大の放物線で、**頂点が自由表面側**にくる（放物線の半分だけを切り取った形）。
...
- **🧱 材力（自重の分布荷重＋自由端）**: $\mu\,u''=-\rho g\sin\theta$ は**自重 $\rho g\sin\theta$ を一様な分布荷重として受ける**形で、材力の自重でたわむ部材と同型。自由表面の条件 $du/dy=0$ は、材力の**自由端でせん断・モーメントが0**になる境界条件に対応する（粘性項はやはり構成則 $\tau=\mu\,du/dy$ の発散）。
- **🔥⚡ 拡散方程式の同型**: 重力を一様ソースとする定常拡散で、片側を固定 $u=0$・片側を**ゼロ流束** $du/dy=0$ にした境界値問題は、**片面加熱・片面断熱**の定常熱伝導（$k\,T''=-\dot q$、断熱端で $dT/dy=0$）と全く同じ形。動粘度 $\nu$ が熱拡散率 $\alpha$ の役で、非対称な境界条件まで含めて同型になる。駆動を圧力勾配に戻せば[ポアズイユ](#/poiseuille-plates)。
