...
表面の各微小面 $dS$ には、圧力による法線力 $-p\,\mathbf{n}\,dS$（圧力は物体を内向きに押すので、外向き法線 $\mathbf{n}$ とは逆向き＝マイナス符号）と、粘性によるせん断力 $\boldsymbol{\tau}\,dS$ が働く。全表面で積分すると合力 $\mathbf{F}$:
...
力 $F$ がどの量で決まるかを並べると $F = f(\rho, U, L, \mu)$（$L$ は物体の代表長さ）。次元は $[F]=\mathrm{ML T^{-2}}$。動圧 $\tfrac12\rho U^2$ は単位面積あたりの力（$[\rho U^2]=\mathrm{M L^{-1}T^{-2}}$）の次元を持つので、これに面積 $A\sim L^2$ を掛けると力の次元になる:

$$
\big[\rho U^2\cdot A\big] = \mathrm{M L^{-1}T^{-2}}\cdot \mathrm{L^2} = \mathrm{M L\,T^{-2}} = [F]
$$

よって力は必ず $F = (\text{無次元数})\times \tfrac12\rho U^2 A$ の形に書ける。$\rho, U, L, \mu$ から作れる独立な無次元数は $\mathrm{Re}=\rho UL/\mu$ ひとつなので、残った無次元数は $\mathrm{Re}$ の関数になる（バッキンガムの $\pi$ 定理）。
...
> **ポイント**: 「力 ＝ 係数 × 動圧 × 面積」。$\tfrac12$ を忘れず、$A$ が前面投影か翼平面かを最初に確認する。係数を出す問題は、まず $C=\dfrac{F}{\frac12\rho U^2 A}$ に数値を入れるだけ。$C_D$ が Re の関数になるのは [レイノルズ数](#/reynolds) の相似則から来ている。
