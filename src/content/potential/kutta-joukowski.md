...
[円柱まわりの流れ](#/cylinder-flow) では上下対称ゆえ揚力ゼロだった。そこに原点まわりの**循環**（自由渦）$-\dfrac{i\Gamma}{2\pi}\ln z$ を重ねると、円柱の上面と下面で流速に差ができ、ベルヌーイより圧力差が生じる。この圧力差の合力が揚力。揚力の大きさは循環 $\Gamma$ と一様流 $U$ だけで決まる — これがクッタ・ジューコフスキーの定理。
...
> **出発点（このページのアンカー）**: 新しい物理は足さない。循環の**定義** $\Gamma=\oint\mathbf{u}\cdot d\mathbf{l}$ に沿った自由渦を1項重ねるだけで、力はブラジウスの公式（検査体積の運動量保存を複素積分に畳んだ形）から取り出す。
...
- **⚡ 電磁気（循環＝周回積分の同型）**: 循環 $\Gamma=\oint\mathbf{u}\cdot d\mathbf{l}$ は、**アンペールの法則** $\oint\mathbf{B}\cdot d\mathbf{l}=\mu_0 I$ と同じ「場を一周線積分すると囲った源を測る」構造。渦度 $\boldsymbol{\omega}=\nabla\times\mathbf{u}$ が磁場 $\mathbf{B}$ の役、渦糸が作る誘導速度はビオ・サバール則と同型で、$\Gamma$ は囲った渦の総量（電流 $I$ に相当）。
- **🌀 流体内（揚力の本体）**: $F_L=\rho U\Gamma$ は翼の形に直接よらず循環だけで決まる。同じ「循環＝揚力」の機構が回転円柱の[マグナス効果](#/magnus)で、形が $\Gamma$ をどう決めるかは[ジューコフスキー変換](#/joukowski-transform)のクッタ条件が担う。土台の対称な流れは[円柱流れ](#/cylinder-flow)。
- **🏗️ 力学（運動量と力）**: ブラジウスの公式は本質的に検査体積の運動量フラックスを複素積分で書いたもの。揚力は「流れの運動量を曲げた反作用」で、噴流が板を押す[運動量の理屈](#/momentum-cv)と同じ系譜にある。

> **ポイント**: 揚力は $\rho,U,\Gamma$ の 3 つだけ・**翼の形に直接よらない**のが定理の凄み。形は「$\Gamma$ をいくつに決めるか」を通してのみ効く（→ クッタ条件、[ジューコフスキー変換](#/joukowski-transform)）。$F_L=\rho U\Gamma$ と「複素速度は $u-iv$」「ブラジウスは $\frac{i\rho}{2}\oint(dW/dz)^2dz$」をセットで暗記。
