...
[速度ポテンシャル φ・流れ関数 ψ](#/potential-stream-func) の $W(z)=\phi+i\psi$、$dW/dz=u-iv$ を使う。

> **出発点（このページのアンカー）**: 新しい物理は何も足さない。ラプラス方程式が**線形**だから成り立つ**重ね合わせ**だけを使い、既知の素解（一様流・ダブレット）を足して境界条件「円柱表面が流線」を満たす、というのがこのページの仕事。
...
- **⚡ 電磁気（双極子の同型）**: 円柱流れを作るダブレット $W_{\text{d}}=\mu/z$ は、**電気双極子の静電ポテンシャル** $\phi_e\propto\dfrac{\cos\theta}{r}$ と同型。一様な外場＋双極子で導体球（円柱）まわりの静電場を作る操作と全く同じく、一様流＋ダブレットで円柱まわりの流れが組める（どちらも $\nabla^2\phi=0$ の重ね合わせ）。
- **🌀 流体内（抗力の起源）**: 完全流体で抗力ゼロ（ダランベールの背理）になるのは前後対称ゆえ。現実の[抗力](#/momentum-deficit)は粘性・剥離が後流を作り対称性を壊すから生じる。揚力をゼロから持ち上げるには、この対称性を**循環**で崩せばよい → [クッタ・ジューコフスキー](#/kutta-joukowski)。
- **🏗️ 力学（圧力分布→合力）**: 表面圧力を法線方向 $\cos\theta$ 成分で一周積分して力を出す手順は、分布した面力を合力に畳む材力・力学の操作そのもの。対称な圧力分布なら合力ゼロという結果も、つり合いの直感どおり。

> **ポイント**: 抗力ゼロは「完全流体には粘性も剥離もないから後流ができず、前後対称」という理想化の帰結。現実の抗力は粘性・剥離が壊す対称性から生じる。揚力をゼロでなくすには、この対称性を**循環**で崩せばよい — それが次の[クッタ・ジューコフスキーの定理](#/kutta-joukowski)。
